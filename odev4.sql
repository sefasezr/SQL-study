...
SELECT * FROM teachers
FULL JOIN courses ON courses.course_name = teachers.course_name;
...
SELECT course_name, start_date FROM courses
...
SELECT firstname, courses.course_name , finish_date FROM courses
INNER JOIN teachers ON courses.course_name = teachers.course_name
WHERE city ='Istanbul'
...
--Kurs ücreti 125 ten fazla olan derslerin isimlerini, kredilerini ve bu dersleri
--veren öğretmenlerin max ve min maaşlarını listeleyiniz

SELECT course_name, credit, (SELECT MAX(salary) FROM teachers WHERE courses.course_name = teachers.course_name), (SELECT MIN(salary) FROM teachers WHERE courses.course_name = teachers.course_name) FROM courses
...
--Yaşı 35 den büyük olan öğretmen varsa,
--bu öğretmenlerin verdiği derslerin isimlerini ve başlangıç tarihlerini listeleyiniz.
...
SELECT course_name, start_date, finish_date FROM courses
WHERE NOT EXISTS (SELECT course_name FROM teachers WHERE teachers.course_name = courses.course_name )

--En az bir öğretmen tarafından verilen derslerin ücretlerini 10 artırınız.
	
UPDATE courses
SET course_fee = course_fee + 10
WHERE EXISTS (SELECT * FROM teachers WHERE teachers.course_name = courses.course_name);

--Ücreti 170 den fazla olan ders varsa bu dersi veren öğretmenlerin tüm bilgilerini
--listeleyiniz.(IN kullanmayınız.)

SELECT * FROM teachers
WHERE EXISTS (SELECT course_name FROM courses WHERE course_fee > 170)
...
UPDATE teachers
SET salary = salary * 2
WHERE age IN (SELECT MIN(age) FROM teachers);
